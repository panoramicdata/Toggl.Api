...
# Progress messages go through Write-Information so that whatever is running this script (a CI job,
# for instance) can capture, suppress or redirect them. -InformationAction Continue keeps them
# visible by default, which Write-Information on its own is not.
...
Write-ProgressMessage "Version: $version"
...
Write-ProgressMessage "Tag $version pushed."
...
Write-ProgressMessage "Waiting for the release run for $version..."
...
Write-ProgressMessage "Run: https://github.com/$repoFullName/actions/runs/$runId"
...
if ($runExitCode -ne 0) {
	...
	Write-ProgressMessage "  gh run rerun $runId --repo $repoFullName --failed"
	exit 1
}

Write-ProgressMessage "Package $version published."